...
export async function POST(request: Request) {
  try {
    ...
    const result = await analyzeBleObservation(user, body as unknown as SubmitBleObservationRequest);
...
function validateBleRequest(body: unknown): void {
  ...
  for (const f of ['riskLevel', 'severity', 'isCamera', 'cameraDetected', 'classification', 'confidence']) {
